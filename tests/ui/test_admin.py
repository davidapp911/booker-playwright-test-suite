...
@pytest.mark.admin
def test_valid_admin_login(page: Page, base_url, credentials):
    page_obj = AdminPage(page)
    page_obj.load_admin_page(base_url)
    page_obj.fill_login_form(**credentials)
    page_obj.submit_login()
    expect(page_obj.room_form()).to_be_visible()


@pytest.mark.admin
def test_invalid_admin_login(page: Page, base_url, credentials):
    page_obj = AdminPage(page)
    page_obj.load_admin_page(base_url)
    page_obj.fill_login_form(credentials["username"], "wrongPassword")
    page_obj.submit_login()
